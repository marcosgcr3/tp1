...
public class Game implements GameModel, GameStatus, GameWorld {
    ...
    private GameConfiguration initialConfig;
    private int level;

    public Game(int level) {
        this.level = level;
        this.initialConfig = FileGameConfiguration.NONE;
        reset(level);
    }
    ...
    public void load(String fileName) throws GameLoadException {
        GameConfiguration fileLoad = new FileGameConfiguration(fileName, this);
        load(fileLoad);
    }
   ...
   public boolean reset(int level) {
        if (this.initialConfig == FileGameConfiguration.NONE) {
            int nuevoNivel;
            if (level == -1) {
